<template>
  <div class="q-pa-md">
    <div v-if="article">
      <h5>Détails de l'article identifiant {{ article.id }}</h5>
      <h2>{{ article.title }}</h2>
      <p>{{ article.body }}</p>
    </div>
    <div v-else>
    ...
    async getSinglePost() {
      ...
      try {
        if (
          ...
          const response = await api.get(`/photos/${this.articleId__}`);
          console.log("-----------------------------");
          console.log(response);
          console.log("-----------------------------");

          const responseD = await api.get(`/posts/${this.articleId__}`);
          console.log("--------------hhhh---------------");
          console.log(responseD);
          console.log("-----------------------------");

          if (response.status === 200) {
            this.article = responseD.data;
          } else {
            this.loading = false;
          }
        }
